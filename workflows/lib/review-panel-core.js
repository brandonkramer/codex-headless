...
export const EVIDENCE_BYTE_BUDGET = 6000
...
export function capUtf8Bytes(text, maxBytes) {
 ...
 * @param {number} [budget]
 */
export function buildEvidencePacket(sections, budget = EVIDENCE_BYTE_BUDGET) {
  const ordered = [
 ...
 */
export function normalizeLenses(raw, maxLenses = MAX_LENSES) {
...
export function normalizeFinding(raw, lensId) {
...
export function findingDedupeKey(finding) {
...
export function normalizeFindings(findings, lensId) {
  return (findings || [])
 ...
 */
export function mergeFindings(lensResults) {
 ...
 */
export function deriveFinalVerdict(lensResults, mergedFindings) {
...
export function buildPrepPrompt(scope, cwd) {
 ...
 */
export function buildLensReviewPrompt(lens, scope, cwd, packet) {
...
export const LENS_REVIEW_SCHEMA = {
  type: 'object',
